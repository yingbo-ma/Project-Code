...
for j in range(len(label_list)):
    ...
    if j == len(label_list) - 1:
        gap = 48000 - len(data_0)
        data_0.tolist()
        data_0.append([0] * gap)
    original_data.append(np.asarray(data_0))

print(len(original_data))

original_data = np.reshape(original_data, (-1, 48000))
...
data.remove([])
data = np.reshape(data, (-1, num_timesteps, len(data_0)))
print(data.shape)
