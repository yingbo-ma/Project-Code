from refined_basic_functions import read_excel, spectrogram_data_prepare, optical_flow_data_prepare, multiple_inputs_model
import numpy as np
...
c_model = multiple_inputs_model(input_shape_A=(PIXEL, PIXEL, SPECTRO_IMAGE_CHANNELS), input_shape_B=(PIXEL, PIXEL, OPTIC_IMAGE_CHANNELS), input_shape_C=(PIXEL, PIXEL, OPTIC_IMAGE_CHANNELS), n_classes=CLASS_NUM)
...
for i in range(ITERATIONS):
    ...
    Acoustic_Y_sup_real = np.concatenate(
        (Acoustic_Y_supervised_samples_class_0, Acoustic_Y_supervised_samples_class_1,
         Acoustic_Y_supervised_samples_class_2), axis=0)
    ...
    left_Motion_Y_sup_real = np.concatenate(
        (left_Motion_Y_supervised_samples_class_0, left_Motion_Y_supervised_samples_class_1,
         left_Motion_Y_supervised_samples_class_2), axis=0)
    ...
    right_Motion_Y_sup_real = np.concatenate(
        (right_Motion_Y_supervised_samples_class_0, right_Motion_Y_supervised_samples_class_1,
         right_Motion_Y_supervised_samples_class_2), axis=0)

    # update model
    c_loss, c_acc = c_model.train_on_batch(x=[Acoustic_Xsup_real, left_Motion_Xsup_real, right_Motion_Xsup_real], y=Acoustic_Y_sup_real)

    if (i + 1) % (BATCH_NUM * 1) == 0:
        epoch += 1
        print(f"Epoch {epoch}, acoustic model accuracy on training data: {c_acc}")
        _, test_acc = c_model.evaluate([spectrogram_test_data, left_optical_flow_test_data, right_optical_flow_test_data], test_label, verbose=0)
        print(f"Epoch {epoch}, c model accuracy on test data: {test_acc}")
        y_pred = c_model.predict([spectrogram_test_data, left_optical_flow_test_data, right_optical_flow_test_data], batch_size=60, verbose=0)
