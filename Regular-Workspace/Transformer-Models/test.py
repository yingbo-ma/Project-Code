...
for j in range(len(train_label_list)):
    ...
    # train_original_data.append(image)
...
for i in range(ITERATIONS):
    ...
    c_loss, c_acc = transform.train_on_batch(X_supervised_samples, Y_supervised_samples)

    if (i + 1) % (BATCH_NUM * 1) == 0:
        epoch += 1
        print(f"Epoch {epoch}, c model accuracy on training data: {c_acc}")
        _, test_acc = transform.evaluate(test_data, test_target, verbose=0)
        print(f"Epoch {epoch}, c model accuracy on test data: {test_acc}")
        y_pred = transform.predict(test_data, batch_size=60, verbose=0)
