...
for sentence_index in range(len(Text_Data)):
    ...
    if Tags_List[sentence_index] == 'ES':
        ...
        sentence_vector.extend(tag_vector)
    # for class 'EO'
    if Tags_List[sentence_index] == 'EO':
        tag_vector = [0, 1, 0, 0, 0, 0, 0, 0, 0]
        sentence_vector.extend(tag_vector)
    # for class 'D'
    if Tags_List[sentence_index] == 'D':
        tag_vector = [0, 0, 1, 0, 0, 0, 0, 0, 0]
        sentence_vector.extend(tag_vector)
    # for class 'Q'
    if Tags_List[sentence_index] == 'Q':
        tag_vector = [0, 0, 0, 1, 0, 0, 0, 0, 0]
        sentence_vector.extend(tag_vector)
    # for class 'U'
    if Tags_List[sentence_index] == 'U':
        tag_vector = [0, 0, 0, 0, 1, 0, 0, 0, 0]
        sentence_vector.extend(tag_vector)
    # for class 'P'
    if Tags_List[sentence_index] == 'P':
        tag_vector = [0, 0, 0, 0, 0, 1, 0, 0, 0]
        sentence_vector.extend(tag_vector)
    # for class 'A'
    if Tags_List[sentence_index] == 'A':
        tag_vector = [0, 0, 0, 0, 0, 0, 1, 0, 0]
        sentence_vector.extend(tag_vector)
    # for class 'OR'
    if Tags_List[sentence_index] == 'OR':
        tag_vector = [0, 0, 0, 0, 0, 0, 0, 1, 0]
        sentence_vector.extend(tag_vector)
    # for class 'OU'
    if Tags_List[sentence_index] == 'OU':
...
main_branch = Bidirectional(LSTM(32, return_sequences=True, activation="relu"))(main_branch)
...
model.compile(
    ...
    loss={"st_output":"binary_crossentropy"},
    metrics=['accuracy']
)
